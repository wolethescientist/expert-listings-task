...
export function buildApp(pool: pg.Pool, options: { logger?: boolean } = {}) {
  ...
  app.post<{ Body: ListingInput }>('/listings', { schema: createListingSchema }, async (request, reply) => {
    const listing = toListing(await createListing(pool, request.body));
    return reply.code(201).header('Location', `/listings/${listing.id}`).send({ data: listing });
  });

  app.get<{ Querystring: PaginationQuery }>('/listings', { schema: listSchema }, async (request) => {
  ...
  app.get<{ Querystring: SearchQuery }>('/listings/search', { schema: searchSchema }, async (request, reply) => {
  ...
  app.get<{ Params: IdParams }>('/listings/:id', { schema: idSchema }, async (request, reply) => {
  ...
  app.patch<{ Params: IdParams; Body: ListingPatch }>('/listings/:id', { schema: { ...idSchema, ...patchListingSchema } }, async (request, reply) => {
    const row = await updateListing(pool, request.params.id, request.body);
    if (!row) return reply.code(404).send({ error: { code: 'LISTING_NOT_FOUND', message: 'Listing not found' } });
    return { data: toListing(row) };
  });

  app.delete<{ Params: IdParams }>('/listings/:id', { schema: idSchema }, async (request, reply) => {
